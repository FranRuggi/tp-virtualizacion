...
function Invoke-GitCompat {
    ...
    # Llamada nativa; capturamos salida y exit code
...
if ($RunDaemon) {
    try {
        ...
        $head   = Invoke-GitCompat -RepoPath $Repo -GitArgs @('rev-parse','HEAD') -LogFile $Log
        $last   = Get-CommitHashFromText $head.Output

        Write-Log -Message "Iniciando demonio para el repositorio: $Repo" -LogFile $Log

        while ($true) {
            $res     = Invoke-GitCompat -RepoPath $Repo -GitArgs @('rev-parse','HEAD') -LogFile $Log
            ...
            if ($null -ne $current -and $current -ne $last) {
                ...
                $df = Invoke-GitCompat -RepoPath $Repo -GitArgs @('diff-tree','--no-commit-id','--name-only','-r','-z','--diff-filter=ACMR',$current) -LogFile $Log
                if ($df.ExitCode -eq 0) {
...
try {
    ...
    # ===== Lanzar hijo pwsh (robusto para espacios / 5.1 y 7+) =====
    ...
    # En 5.1 y 7+, Start-Process con -ArgumentList <string> funciona
